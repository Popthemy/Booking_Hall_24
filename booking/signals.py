...
1. import the post_save method from models.signal 'from django.db.models.signals import pre_save'
...
def date_time_hallUnique(sender, instance, **kwargs):
    ...
    hall_date = Main_Schedule.objects.filter(pre_schedule__hall=hall, pre_schedule__date=date).exclude(pk=instance.pk if instance.pk else None)
