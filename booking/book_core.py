...
def dynamic_time_choices(start_end):
    ...
    return: if the queryset is empty then all time is available at that moment TIME_CHOICES 
    else return avail and unavailable which avail is needed by the field as the time choice and 
    unavailable needed to check for conflict inside of our views or signal to make sure that the time user choose doesn't span in the middle of a class .'''

    # total range of time for a day class range is from 7am to 7pm
    TIME_CHOICES = [(H, f'{H-12 if H >12 else 12  }:00 PM') if H >=
                    12 else (H, f'{H}:00 AM') for H in range(MIN_TIME, MAX_TIME)]
    ...
    unavailable_time = {time for start,
                        end in start_end for time in range(start, end)}

    # to get available time based on the previous time in our Db
    available_time = [
        choices for choices in TIME_CHOICES if choices[0] not in unavailable_time]
...
def course_span_conflict(starts, ends, start_end):
    ...
    span_conflict = any(time in unavailable_time for time in range(
        starts, ends))  # return false if all are false
